...
//		tv.MIN_VOLUME = 5;	// 상수이므로 변경 불가능
		tv.turnOn();
		tv.setVolume(10);	// 추상 메소드
		tv.setMute(true);	// 무음 처리 (default 메소드)
		...
		rc.setMute(true);
		rc.setVolume(20);
//		rc.changeChannel(07);	// 불가능 (Television
								// 클래스에서 정의한 메소드)
//		rc.channel = 8;	// 불가능 (클래스에서 정의)
			// 내가 볼수있는 것은 인터페이스에서 정의한 것만
		...
		if(rc instanceof Television) {
			...
			// ClassCastException 예외가 발생함 (원래 Television임)
		}
	}
